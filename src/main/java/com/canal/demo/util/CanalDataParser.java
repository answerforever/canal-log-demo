...
public class CanalDataParser {
    ...
    static {
        ...
        row_format = SEP
                + "----------------> binlog[{}:{}] , name[{},{}] , eventType : {} , executeTime : {}({}) , gtid : " +
                "({}) , delay : {} ms"
                + SEP;
    ...
    public static class ThreeTuple<A, B, C> {
        public final A eventType;
        public final B columnMapBefore;
        public final C columnMapAfter;
     ...
     * @return
     */
    public static List<ThreeTuple<OperationType, Map, Map>> printEntry(List<Entry> entrys) {
        List<ThreeTuple<OperationType, Map, Map>> rows = new ArrayList<>();
        ...
        for (Entry entry : entrys) {
            ...
            if (entry.getEntryType() == EntryType.ROWDATA) {
                ...
                for (RowData rowData : rowChage.getRowDatasList()) {
                    ...
                    List<Column> columnsBefore = new ArrayList<>();
                    // 事件类型是delete返回删除前的列内容，否则返回改变后列的内容
                    if (eventType == EventType.UPDATE || eventType == EventType.DELETE) {
                        columnsBefore = rowData.getBeforeColumnsList();
                    }
                    if (eventType != EventType.DELETE) {
                        columns = rowData.getAfterColumnsList();
                    }
                    HashMap<String, Object> mapAfter = new HashMap<>(16);
                    HashMap<String, Object> mapBefore = new HashMap<>(16);
                    // 循环把列的name与value放入map中
                    for (Column column : columns) {
                        mapAfter.put(column.getName(), column.getValue());
                    }
                    String tableName = entry.getHeader().getTableName();
                    OperationType operationType = new OperationType();
                    operationType.setEventType(eventType);
                    operationType.setTableName(tableName);
                    for (Column columnBefore : columnsBefore) {
                        mapBefore.put(columnBefore.getName(), columnBefore.getValue());
                    }
                    rows.add(new ThreeTuple<>(operationType, mapBefore, mapAfter));
                }
            }
        }
        return rows;
